...
public class FontChanger : EditorWindow
{
        ...
        if (GUILayout.Button("Replace All Fonts"))
        {
            if (newFont == null)
            {
                EditorUtility.DisplayDialog(
                    "Error",
                    "Please assign a New Font Asset first!",
                    "OK"
                );
                return;
            }
    ...
    private void ReplaceFonts()
    {
        ...
        if (changedCount > 0)
        {
            ...
            Debug.Log(
                $"Successfully replaced fonts on {changedCount} TextMeshPro objects."
            );

            EditorUtility.DisplayDialog(
                "Success",
                $"Replaced fonts on {changedCount} objects!",
                "OK"
            );
        }
        else
        {
            Debug.LogWarning(
                "No TextMeshPro objects found in the active scene."
            );

            EditorUtility.DisplayDialog(
                "Finished",
                "No TextMeshPro objects found in this scene.",
                "OK"
            );
        }
    }
